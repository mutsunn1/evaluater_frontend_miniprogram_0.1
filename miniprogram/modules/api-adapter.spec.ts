...
function setupWx(successData?: unknown, statusCode = 200) {
  mockWxRequest = vi.fn().mockImplementation((opts: Record<string, unknown>) => {
    const successCb = opts.success as ((res: Record<string, unknown>) => void) | undefined;
    const failCb = opts.fail as ((err: Record<string, unknown>) => void) | undefined;
    queueMicrotask(() => {
...
function mockSseParser(events: Array<{ type: string; data: Record<string, unknown> }>) {
  vi.doMock('./sse-parser', () => ({
    startSseRequest(
      _opts: unknown,
      config: {
        onEvent: (type: string, data: Record<string, unknown>) => boolean | void;
...
describe('api-adapter — non-streaming', () => {
  ...
  it('getConfidence calls GET to correct endpoint', async () => {
    setupWx({ accuracy: 0.8, confidence: 0.9, sample_size: 10, should_stop: false });
...
describe('api-adapter — streaming', () => {
  ...
  it('streamQuestion collects multiple question events', async () => {
    mockSseParser([
      ...
      { type: 'question', data: { question: { question_type: 'multiple_choice', question_text: 'Q1', scene: '', grammar_focus: '', target_level: '' }, batch_id: 'b1', batch_index: 0, skill_dimension: 'vocabulary' } },
      { type: 'question', data: { question: { question_type: 'true_false', question_text: 'Q2', scene: '', grammar_focus: '', target_level: '' }, batch_id: 'b1', batch_index: 1, skill_dimension: 'reading' } },
    ]);
    const { streamQuestion } = await import('./api-adapter');
    const onThinking = vi.fn();
    const result = await streamQuestion('s1', onThinking, { requestId: 'req-123' });
  ...
  it('streamQuestion rejects when stream ends without question data', async () => {
    ...
    const { streamQuestion } = await import('./api-adapter');
    await expect(streamQuestion('s1', vi.fn())).rejects.toThrow(/without question/);
  });

  it('batchSubmitAnswer resolves with answer event', async () => {
    mockSseParser([
      { type: 'answer', data: { results: [{ item_id: 1 }], confidence: 0.9, accuracy: 0.85, auto_stop: false } },
    ]);
    const { batchSubmitAnswer } = await import('./api-adapter');
    const onThinking = vi.fn();
    const result = await batchSubmitAnswer('s1', [{ question_index: 0, answer: 'x' }], onThinking, { submissionId: 'sub-1' });
    expect(result.results).toHaveLength(1);
  });

  it('streamSubmitAnswer resolves with answer event', async () => {
    mockSseParser([
      { type: 'answer', data: { item_id: 1, is_correct: true, feedback: 'good', confidence: 0.9, accuracy: 0.8 } },
    ]);
  ...
  it('streamColdStart resolves with question event', async () => {
    mockSseParser([
      { type: 'question', data: { cold_start: true, round: 1, label: 'intro', question: 'Tell me...' } },
    ]);
  ...
  it('streamColdStartAnswer resolves with answer event', async () => {
    mockSseParser([
      { type: 'answer', data: { cold_start_complete: false, feedback: 'nice' } },
    ]);
    const { streamColdStartAnswer } = await import('./api-adapter');
    const onThinking = vi.fn();
    const result = await streamColdStartAnswer('s1', 'cs answer', 2500, onThinking);
    expect(result.feedback).toBe('nice');
  });

  it('streaming rejects on error event', async () => {
    mockSseParser([
      { type: 'error', data: { message: 'backend error' } },
    ]);
    const { streamQuestion } = await import('./api-adapter');
    await expect(streamQuestion('s1', vi.fn())).rejects.toThrow(/backend error/);
  });
  ...
  it('streaming rejects on stream complete with error', async () => {
    ...
    const { streamQuestion } = await import('./api-adapter');
    await expect(streamQuestion('s1', vi.fn())).rejects.toThrow(/connection lost/);
  });
  ...
  it('abort signal rejects the promise', async () => {
    vi.doMock('./sse-parser', () => ({
      startSseRequest(
        _opts: unknown,
        config: { onComplete?: (err?: Error) => void; signal?: { aborted: boolean } },
      ) {
    ...
    const { streamQuestion } = await import('./api-adapter');
    await expect(streamQuestion('s1', vi.fn(), { signal: { aborted: true } })).rejects.toThrow(/aborted/);
  });
});
